...
[leetcode链接](https://leetcode.cn/problems/maximum-length-of-repeated-subarray/)

*关键词* ：动态规划，滑动窗口

这题一开始想得有点偏了，想用map套map的方法做，但是越想越复杂。答案用的动态规划做法不算太难，不过这里状态变量的选取还是值得学习的，一开始自己的想法是，为了能实现O(m+n)的复杂度，选择以元素`nums1[i]`为结尾的重复最长子数组的长度，但是发现这种选法会漏一些情况，而答案的做法则是O(mn)的时间复杂度，对于数组`nums1`中从左向右的每个元素，从右向左遍历数组`nums2`，比较`nums2`数组各位置的后缀与数组`nums1`中这个位置的后缀重复的长度的最大值，比较后缀的过程可以复用之前的计算结果。而另外一个滑动窗口的做法更巧妙，两个数组一旦有重复子数组，那么这个重复子数组一定对应着这两个数组的一种重合方式，滑动窗口的做法就是遍历两个数组的所有可能的重合方式，对于每一种重合方式，遍历两个数组的重合部分，就一定能找到最长的重复子数组。
...
最近的动态规划题做起来有点吃力啊，一开始做这题的时候一直想着沿用上一题的思路，但是实际证明不太行，因为上一题是要找子数组，也就是说必须是要连续的，所以取最长重复后缀作为状态变量是有意义的，但是这题是找最长子序列，所以不能那么做。另外的比较合理的状态变量就是`text1`的前`i`个字符与`text2`的前`j`个字符中的最长子序列，有了这个状态变量就要考虑递推公式，如果`text1[i] == text2[j]`，这种情况是比较容易的，很自然的有`dp[i][j] = dp[i-1][j-1] + 1`；如果`text1[i] != text2[j]`，这种情况比较麻烦，不过考虑到两个字符串的对称，也不难想到应该有`dp[i][j] = Math.max(dp[i][j-1], dp[i-1][j])`。这样就能解决这个问题了，但是为了降低空间复杂度，需要用到滚动数组，这又带来了新的问题，不妨假设有`text1.length() >= text2.length()`，在固定了外层遍历的`i`后，内层遍历的方向怎么都不对，因为如果从左向右遍历，那么就只能取到`dp[i][j-1]`，如果从右向左遍历，就只能取到`dp[i-1][j-1]`，但是根据前面的分析，这两个量我们在递推过程中都是要用到的，因此为解决这个问题，需要建立一个变量来临时储存一个值，比如从左向右遍历的过程中，可以维护一个变量，使其始终储存着`dp[i-1][j-1]`的值。

### 20220616，不相交的线

[leetcode链接](https://leetcode.cn/problems/uncrossed-lines/)

*关键词* ：动态规划

最长公共子序列问题换了个说法，不过从这个题想到公共子序列还是不简单的，要不是刚做过那题估计也很难想到，不过做法上基本是一模一样的。
...
还是自己把问题想复杂了，一看这题就想用双指针，实际上后面那个指针是完全没起到作用的，只需要维护一个指针和一个和就足够了。

### 20220618，判断子序列

[leetcode链接](https://leetcode.cn/problems/is-subsequence/)

*关键词* ：双指针，动态规划

最近做题不太顺啊，一开始想的是和前面判断两个数组最大公共子序列的做法，就是把原来的递推公式改一下，把状态数组改为`boolean`类型，做是可以做，但是时间复杂度太高了。双指针的做法也考虑过，后来否决了，是因为，例如`s="abc", t="ahbadcs"`，其中在`t`中第二次遍历到`a`的时候要怎么处理的问题，但是看了答案发现这个问题是不需要考虑的，因为要找子序列，如果`t`中的第二个`a`及之后有满足要求的子序列，那么把这个子序列里的`a`换成`t`里的第一个`a`照样能满足要求，因此在遍历的时候，在`t`中找到了`s`中的某一个元素之后，就可以直接开始找`s`中的下一个元素。另外答案中用到的动态规划的做法也很值得学习，原理上有借鉴KMP算法，不过这里是记录从每一个位置之后每一个字母第一次出现的位置，这个动态规划数组构建起来很容易，后面再遍历`s`，只需要维护一个变量，记录当前的起始位置即可。